...
def clean_text(text, fashion_terms):
    ...
    text = text.translate(str.maketrans("", "", string.punctuation)) #Transalation table to make any punctuation into nothing
    words = word_tokenize(text) #Spltis all words into separate items in a list.
    
    #Get stopwords from NLTK English stopwords
    stop_words = set(stopwords.words('english'))
    
    #Clean it so there are no stop words, all are alphabets and the word is in the fashion_terms list.
    cleaned_words = [
...
full_text = " ".join(all_content) #From the JSON file, add all the content to the same text string
